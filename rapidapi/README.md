...
Expose port 8080 behind managed HTTPS. Enforce an ingress body limit of 6 MiB, bounded concurrency and a hosting budget. Configure RapidAPI's Base URL only after checking HTTPS, gateway authentication, valid/invalid requests and actual quota enforcement through RapidAPI. The image was built and exercised successfully in [GitHub Actions](https://github.com/QIU-Guanzong/delivery-check/actions/runs/34670800986) on 2026-09-12 at commit 061bf48. The smoke run checks a non-root, read-only container, health, gateway refusal, PASS/FAIL reports, invalid contracts and malformed JSON. This verifies the container, not a public HTTPS deployment or RapidAPI forwarding.
